...
FillCHOPPluginInfo(CHOP_PluginInfo *info)
{
	...
	info->customOPInfo.authorName->setString("Jace");
	info->customOPInfo.authorEmail->setString("ja.weyant0@gmail.com");
...
CreateCHOPInstance(const OP_NodeInfo* info)
{
	...
	return new QuaternionCHOP(info);
}
...
DestroyCHOPInstance(CHOP_CPlusPlusBase* instance)
{
	...
	delete (QuaternionCHOP*)instance;
}

};


QuaternionCHOP::QuaternionCHOP(const OP_NodeInfo* info) : myNodeInfo(info)
{
	myExecuteCount = 0;
	myOffset = 0.0;
}

QuaternionCHOP::~QuaternionCHOP()
{

}

void
QuaternionCHOP::getGeneralInfo(CHOP_GeneralInfo* ginfo, const OP_Inputs* inputs, void* reserved1)
{
...
bool
QuaternionCHOP::getOutputInfo(CHOP_OutputInfo* info, const OP_Inputs* inputs, void* reserved1)
{
...
void
QuaternionCHOP::getChannelName(int32_t index, OP_String *name, const OP_Inputs* inputs, void* reserved1)
{
	name->setString("chan1");
}

void
QuaternionCHOP::execute(CHOP_Output* output,
		...
		if (isCombined)
		{
			...
			for (int i = 0; i < output->numChannels; i++)
			{
				for (int j = 0; j < output->numSamples; j++)
				{
					...
					{
						total += float(cinput->getChannelData(k)[ind] * scale);
					}
...
int32_t
QuaternionCHOP::getNumInfoCHOPChans(void * reserved1)
{
...
void
QuaternionCHOP::getInfoCHOPChan(int32_t index,
...
bool		
QuaternionCHOP::getInfoDATSize(OP_InfoDATSize* infoSize, void* reserved1)
{
	infoSize->rows = 2;
	infoSize->cols = 2;
...
void
QuaternionCHOP::getInfoDATEntries(int32_t index,
										int32_t nEntries,
...
void
QuaternionCHOP::setupParameters(OP_ParameterManager* manager, void *reserved1)
{
	// speed
	{
	...
	// initial pt
	{
		...
		np.name = "Pt";
		np.label = "Initial Point";
		np.defaultValues[0] = 0.0f;
		np.defaultValues[1] = 0.0f;
		np.defaultValues[2] = 1.0f;

		for (int i = 0; i < 3; i++) {
			np.minValues[i] = -10.0f;
			np.maxValues[i] = 10.0f;
			np.minSliders[i] = -10.0f;
			np.maxSliders[i] = 10.0f;
			np.clampMins[i] = false;
			np.clampMaxes[i] = false;
		}
...
void 
QuaternionCHOP::pulsePressed(const char* name, void* reserved1)
{
